...
symm A B = wiring (A ⊗ B) (B ⊗ A) ⊎-swap lemma where
...
assoc⁻¹ A B C = wiring (A ⊗ (B ⊗ C)) ((A ⊗ B) ⊗ C) ⊎-assoc lemma where
  ...
  lemma : 
    (Assertions (⟨ABox⟩ ⊎-assoc (iface ((A ⊗ B) ⊗ C)))) ⊆
      (Assertions (iface (A ⊗ (B ⊗ C))))
  lemma (inj₁ (inj₁ a∈A)) = inj₁ (subst₂ Assertions 
  ...
  lemma (inj₁ (inj₂ b∈B)) = inj₂ (inj₁ (subst₂ Assertions 
    (trans (⟨ABox⟩-resp-∘² ⊎-assoc inj₁ inj₂ (iface B))
      (sym (⟨ABox⟩-resp-∘ inj₂ inj₁ (iface B)))) 
    refl b∈B))
  lemma (inj₂ c∈C) = inj₂ (inj₂ (subst₂ Assertions
...
unit₁ A = wiring (I ⊗ A) A inj₂ inj₂

unit₁⁻¹ : ∀ {S T : TBox Σ} (A : Object S T) → (A ⇒ (I ⊗ A))
unit₁⁻¹ A = wiring A (I ⊗ A) ⊎-unit₁ lemma where

  lemma : Assertions (⟨ABox⟩ ⊎-unit₁ (iface (I ⊗ A))) ⊆ Assertions (iface A)
  lemma (inj₁ ())
  lemma (inj₂ a∈A) = subst₂ Assertions 
    (trans (⟨ABox⟩-resp-∘ ⊎-unit₁ inj₂ (iface A)) (⟨ABox⟩-resp-id (iface A)))
    refl a∈A

unit₂ : ∀ {S T : TBox Σ} (A : Object S T) → ((A ⊗ I) ⇒ A)
unit₂ A = wiring (A ⊗ I) A inj₁ inj₁

unit₂⁻¹ : ∀ {S T : TBox Σ} (A : Object S T) → (A ⇒ (A ⊗ I))
unit₂⁻¹ A = wiring A (A ⊗ I) ⊎-unit₂ lemma where

  lemma : Assertions (⟨ABox⟩ ⊎-unit₂ (iface (A ⊗ I))) ⊆ Assertions (iface A)
  lemma (inj₁ a∈A) = subst₂ Assertions 
    (trans (⟨ABox⟩-resp-∘ ⊎-unit₂ inj₁ (iface A)) (⟨ABox⟩-resp-id (iface A)))
    refl a∈A
  lemma (inj₂ ())
