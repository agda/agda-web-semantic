...
TSkolems (C ⊑₁ D) = (CSkolems (neg C)) × (CSkolems D)
TSkolems (Q ⊑₂ R) = True

tskolem : ∀ T → (TSkolems T) → Formula Σ
tskolem ε Φ = true
tskolem (T , U) (Φ , Ψ) = (tskolem T Φ) ∧ (tskolem U Ψ)
tskolem (C ⊑₁ D) (Φ , Ψ) = ∀₁ λ x → (cskolem (neg C) Φ x) ∨ (cskolem D Ψ x)
...
tskolem-sound I (C ⊑₁ D) (Φ , Ψ) I✓ I⊨C⊑D = lemma where
  lemma : ∀ {x} → (x ∈ I ⟦ C ⟧₁) → (x ∈ I ⟦ D ⟧₁)
  lemma {x} x∈⟦C⟧ with I⊨C⊑D x
  lemma {x} x∈⟦C⟧ | inj₁ x∈⟦¬C⟧ = ⊥-elim (neg-sound I C (cskolem-sound I (neg C) Φ x I✓ x∈⟦¬C⟧) x∈⟦C⟧)
  lemma {x} x∈⟦C⟧ | inj₂ x∈⟦D⟧  = cskolem-sound I D Ψ x I✓ x∈⟦D⟧
tskolem-sound I (Q ⊑₂ R) Φ I✓ I⊨Q⊑R = lemma where
  lemma : ∀ {xy} → (xy ∈ I ⟦ Q ⟧₂) → (xy ∈ I ⟦ R ⟧₂)
  lemma {x , y} xy∈⟦Q⟧ = rskolem-sound I R x y I✓ (I⊨Q⊑R x y (rskolem-complete I Q x y I✓ xy∈⟦Q⟧))
