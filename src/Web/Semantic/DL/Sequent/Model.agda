...
open import Web.Semantic.DL.ABox.Interp.Morphism using ( _≲_ ; _,_ )
...
open import Web.Semantic.DL.Sequent using 
  ( _⊢_∼_ ; _⊢_∈₁_ ; _⊢_∈₂_ ; ∼-assert ; ∼-refl ; ∼-sym ; ∼-trans ; ∼-≤1
  ; ∈₂-assert ; ∈₂-resp-∼ ; ∈₂-subsum ; ∈₂-inv-I ; ∈₂-inv-E
  ; ∈₁-assert ; ∈₁-resp-∼ ; ∈₁-subsum ; ∈₁-⊤-I ; ∈₁-⊓-I ; ∈₁-⊓-E₁ ; ∈₁-⊓-E₂ 
  ; ∈₁-⊔-I₁ ; ∈₁-⊔-I₂ ; ∈₁-∃-I ; ∈₁-∀-E )
open import Web.Semantic.DL.Signature using ( Signature )
open import Web.Semantic.DL.TBox using ( TBox ; Axioms ; ε ; _,_ ;_⊑₁_ ; _⊑₂_ )
open import Web.Semantic.DL.TBox.Interp using 
  ( interp ; _⊨_≈_ ; ≈-refl ; ≈-sym ; ≈-trans ) renaming
  ( Interp to Interp′ )
open import Web.Semantic.DL.TBox.Interp.Morphism using ( morph )
...
minimal′ : (K : KB Σ X) → Interp′ Σ
minimal′ K = 
  interp X (λ x y → (K ⊢ x ∼ y)) ∼-refl ∼-sym ∼-trans
    (λ c x → K ⊢ x ∈₁ ⟨ c ⟩) (λ r xy → K ⊢ xy ∈₂ ⟨ r ⟩)
    (λ c → ∈₁-resp-∼) (λ r → ∈₂-resp-∼)

minimal : (K : KB Σ X) → Interp Σ X
minimal K = (minimal′ K , id)
...
minimal-model : ∀ {K} → (tbox K ∈ μTBox) → (minimal K ⊨ K)
minimal-model {K} μT = 
  ...
  minimal-abox : ∀ A → (Assertions A ⊆ Assertions (abox K)) → minimal K ⊨a A
...
minimal-minimal K (I , i) (I⊨T , I⊨A) = 
  (morph i minimal-≈ minimal-con minimal-rol , λ x → ≈-refl I) where 

    mutual

      minimal-≈ : ∀ {x y} → (K ⊢ x ∼ y) → (I ⊨ i x ≈ i y)
      minimal-≈ (∼-assert x∼y∈A)       = Assertions✓ (I , i) (abox K) x∼y∈A I⊨A
      minimal-≈ ∼-refl                 = ≈-refl I
      ...
      minimal-con (∈₁-assert x∈C∈A)           = Assertions✓ (I , i) (abox K) x∈C∈A I⊨A
      ...
      minimal-rol (∈₂-assert xy∈R∈A)                   = Assertions✓ (I , i) (abox K) xy∈R∈A I⊨A
